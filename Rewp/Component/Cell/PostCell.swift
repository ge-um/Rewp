...
final class PostCell: UITableViewCell, IsIdentifiable {
    ...
    private func setupUI() {
        ...
        interactionContainer.flex
            ...
            .alignItems(.center)
            .define { flex in
                flex.addItem(likeIconImageView).size(16)
                flex.addItem(likeCountLabel).marginLeft(4)
                flex.addItem(commentIconImageView).size(16).marginLeft(12)
                flex.addItem(commentCountLabel).marginLeft(4)
            }
    }
