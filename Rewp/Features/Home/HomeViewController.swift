...
class HomeViewController: UIViewController {
    ...
    private func testValidateEmail() {
        userRepository
            .validateEmail("99999@ggg.com")
            .observe(on: MainScheduler.instance)
            .subscribe(
                onSuccess: { response in
                    print("✅ ValidateEmail Success:")
                    print("Message: \(response.message)")
                },
                onFailure: { error in
                    print("❌ ValidateEmail Error: \(error)")
                }
            )
            .disposed(by: disposeBag)
    }

    private func testJoin() {
        let joinRequest = JoinRequest(
            email: "99999@example.com",
            password: "Test1234!@",
            nick: "테스트유저",
    ...
    private func testLogin() {
        userRepository
            .login(email: "99999@example.com", password: "Test1234!@")
            .observe(on: MainScheduler.instance)
            .subscribe(
                onSuccess: { response in
                    print("✅ Login Success:")
